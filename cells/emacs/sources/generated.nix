# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  acm-terminal = {
    pname = "acm-terminal";
    version = "1851d8fa2a27d3fd8deeeb29cd21c3002b8351ba";
    src = fetchFromGitHub {
      owner = "twlz0ne";
      repo = "acm-terminal";
      rev = "1851d8fa2a27d3fd8deeeb29cd21c3002b8351ba";
      fetchSubmodules = false;
      sha256 = "sha256-EYhFrOo0j0JSNTdcZCbyM0iLxaymUXi1u6jZy8lTOaY=";
    };
    date = "2023-12-06";
  };
  activity-watch-mode = {
    pname = "activity-watch-mode";
    version = "19aed6ca81a3b1e549f47867c924d180d8536791";
    src = fetchgit {
      url = "https://github.com/pauldub/activity-watch-mode";
      rev = "19aed6ca81a3b1e549f47867c924d180d8536791";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-FZmWuu64D08kysLa/LU8o3W60s6/0FeQbVL923mJd1k=";
    };
    date = "2024-03-13";
  };
  all-the-icons = {
    pname = "all-the-icons";
    version = "ee414384938ccf2ce93c77d717b85dc5538a257d";
    src = fetchgit {
      url = "https://github.com/domtronn/all-the-icons.el";
      rev = "ee414384938ccf2ce93c77d717b85dc5538a257d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-Da+luefWlhANKNNLa29pJpCez+O2IOYUNDUlqF0486E=";
    };
    date = "2024-01-08";
  };
  chemacs = {
    pname = "chemacs";
    version = "c2d700b784c793cc82131ef86323801b8d6e67bb";
    src = fetchgit {
      url = "https://github.com/plexus/chemacs2";
      rev = "c2d700b784c793cc82131ef86323801b8d6e67bb";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-/WtacZPr45lurS0hv+W8UGzsXY3RujkU5oGGGqjqG0Q=";
    };
    date = "2023-01-20";
  };
  color-rg = {
    pname = "color-rg";
    version = "38d60658494ea3093ff3121458736a2eb6ab2b1b";
    src = fetchgit {
      url = "https://github.com/manateelazycat/color-rg";
      rev = "38d60658494ea3093ff3121458736a2eb6ab2b1b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-BVf5Dajx8r85M4TlsOih0feLeDUcmW7pGrPV/WLsiyY=";
    };
    date = "2024-05-12";
  };
  copilot-el = {
    pname = "copilot-el";
    version = "733bff26450255e092c10873580e9abfed8a81b8";
    src = fetchgit {
      url = "https://github.com/zerolfx/copilot.el.git";
      rev = "733bff26450255e092c10873580e9abfed8a81b8";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-Knp36PtgA73gtYO+W1clQfr570bKCxTFsGW3/iH86A0=";
    };
    date = "2024-05-01";
  };
  doom-emacs = {
    pname = "doom-emacs";
    version = "9620bb45ac4cd7b0274c497b2d9d93c4ad9364ee";
    src = fetchgit {
      url = "https://github.com/hlissner/doom-emacs";
      rev = "9620bb45ac4cd7b0274c497b2d9d93c4ad9364ee";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = true;
      sha256 = "sha256-4F3WaKetjPyMxntP/dmRNMCCDjnHmx3sdnvc6Idmms0=";
    };
    date = "2024-04-18";
  };
  grip = {
    pname = "grip";
    version = "a3f0ab5c8942ac15cf68f790b80c0a43b3a4c71e";
    src = fetchgit {
      url = "https://github.com/joeyespo/grip";
      rev = "a3f0ab5c8942ac15cf68f790b80c0a43b3a4c71e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-ZOkuzGXgNJw9tj7/MATBojEmDszK7FcYyYKy59Qo19I=";
    };
    date = "2023-10-12";
  };
  jinx = {
    pname = "jinx";
    version = "3c36f1eb31713869ffbdbf55971671efa4f01966";
    src = fetchFromGitHub {
      owner = "minad";
      repo = "jinx";
      rev = "3c36f1eb31713869ffbdbf55971671efa4f01966";
      fetchSubmodules = false;
      sha256 = "sha256-MYk8+mptPhFeLLJk4rk37jCbJ1NKErLKDjZnEE7zyRE=";
    };
    date = "2024-05-15";
  };
  lsp-bridge = {
    pname = "lsp-bridge";
    version = "2f59043c21c3f1a560e30352dd964549fadf6797";
    src = fetchFromGitHub {
      owner = "manateelazycat";
      repo = "lsp-bridge";
      rev = "2f59043c21c3f1a560e30352dd964549fadf6797";
      fetchSubmodules = false;
      sha256 = "sha256-KQOCc+wxmiCMrOCuXmGKdwg3Iq5gxXoKCB0Sic2kUxc=";
    };
    date = "2024-05-22";
  };
  nix-mode = {
    pname = "nix-mode";
    version = "719feb7868fb567ecfe5578f6119892c771ac5e5";
    src = fetchgit {
      url = "https://github.com/NixOS/nix-mode";
      rev = "719feb7868fb567ecfe5578f6119892c771ac5e5";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-W/bxCAn4cvzKGbhILxtTiWJ5Wln2w6g4TPgk+ziCXsQ=";
    };
    ename = "nix-mode";
    date = "2023-04-21";
  };
  nix-ts-mode = {
    pname = "nix-ts-mode";
    version = "63d83d7fa721b1b27cac330beb5cfcbbcc808196";
    src = fetchFromGitHub {
      owner = "remi-gelinas";
      repo = "nix-ts-mode";
      rev = "63d83d7fa721b1b27cac330beb5cfcbbcc808196";
      fetchSubmodules = false;
      sha256 = "sha256-yKctP4tv5dvVce/r+NIbCdt066hpxt0FQeCNyEr+iu4=";
    };
    date = "2024-05-23";
  };
  org-pretty-table = {
    pname = "org-pretty-table";
    version = "38e4354bbf7a8d08294babd067fac697038119b1";
    src = fetchgit {
      url = "https://github.com/Fuco1/org-pretty-table";
      rev = "38e4354bbf7a8d08294babd067fac697038119b1";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-IeHnoB9hpISwuzI+pKiU+5wvGvz9YJrG5amGg6DGgq4=";
    };
    date = "2023-03-19";
  };
  org-protocol-capture-html = {
    pname = "org-protocol-capture-html";
    version = "a912aaefae8abdada2b2479aec0ad53fcf0b57bf";
    src = fetchgit {
      url = "https://github.com/alphapapa/org-protocol-capture-html";
      rev = "a912aaefae8abdada2b2479aec0ad53fcf0b57bf";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-X+t8E+0Bxtl6YMU0SwA/vRD8kn1lSXd2PcBhIY001Xo=";
    };
    date = "2022-10-05";
  };
}
