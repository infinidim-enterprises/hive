# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  aider = {
    pname = "aider";
    version = "v0.13.2";
    src = fetchFromGitHub {
      owner = "tninja";
      repo = "aider.el";
      rev = "v0.13.2";
      fetchSubmodules = false;
      sha256 = "sha256-YBgnGDLhKZohSNnNaOjV0QiJ3BtBhG9T7fUU4CJ8YqQ=";
    };
  };
  all-the-icons = {
    pname = "all-the-icons";
    version = "4778632b29c8c8d2b7cd9ce69535d0be01d846f9";
    src = fetchFromGitHub {
      owner = "domtronn";
      repo = "all-the-icons.el";
      rev = "4778632b29c8c8d2b7cd9ce69535d0be01d846f9";
      fetchSubmodules = false;
      sha256 = "sha256-3OhM+dEzAYDlvJL4bbvQMxherEwyC3wKC7Tjb168J6Q=";
    };
    date = "27 May 2025";
  };
  cape = {
    pname = "cape";
    version = "2.4";
    src = fetchFromGitHub {
      owner = "minad";
      repo = "cape";
      rev = "2.4";
      fetchSubmodules = false;
      sha256 = "sha256-sOI2NWCCPFWiCEC4DMnWpb1f9/p8hc9cznYkzXeOV2g=";
    };
  };
  chemacs = {
    pname = "chemacs";
    version = "c2d700b784c793cc82131ef86323801b8d6e67bb";
    src = fetchFromGitHub {
      owner = "plexus";
      repo = "chemacs2";
      rev = "c2d700b784c793cc82131ef86323801b8d6e67bb";
      fetchSubmodules = false;
      sha256 = "sha256-/WtacZPr45lurS0hv+W8UGzsXY3RujkU5oGGGqjqG0Q=";
    };
    date = "20 Jan 2023";
  };
  color-rg = {
    pname = "color-rg";
    version = "41ad8fda5b6579a841889b2395c26cd642a006de";
    src = fetchFromGitHub {
      owner = "manateelazycat";
      repo = "color-rg";
      rev = "41ad8fda5b6579a841889b2395c26cd642a006de";
      fetchSubmodules = false;
      sha256 = "sha256-wmFaEDvLNMM0Fk6g0eGBHTzqqOf1Qxne/yMc1rGTrZM=";
    };
    date = "07 Mar 2025";
  };
  color-theme-sanityinc-solarized = {
    pname = "color-theme-sanityinc-solarized";
    version = "f42431850e0ff0cff90c6cc39edc222faa40323d";
    src = fetchFromGitHub {
      owner = "purcell";
      repo = "color-theme-sanityinc-solarized";
      rev = "f42431850e0ff0cff90c6cc39edc222faa40323d";
      fetchSubmodules = false;
      sha256 = "sha256-CImGELoSOfvbGPQvl99jcCMksOmc4sGw//JIX5/K4pw=";
    };
    date = "26 Nov 2024";
  };
  copilot-el = {
    pname = "copilot-el";
    version = "7ee4758bb748beac7d29e62de5d2e752ebafb858";
    src = fetchFromGitHub {
      owner = "zerolfx";
      repo = "copilot.el";
      rev = "7ee4758bb748beac7d29e62de5d2e752ebafb858";
      fetchSubmodules = false;
      sha256 = "sha256-jzrf6k9yJEMR0H1hOkf1LY2p5ik++poMoAlrhrYPD2U=";
    };
    date = "10 Dec 2025";
  };
  corfu = {
    pname = "corfu";
    version = "2.7";
    src = fetchFromGitHub {
      owner = "minad";
      repo = "corfu";
      rev = "2.7";
      fetchSubmodules = false;
      sha256 = "sha256-1d9cISNbrK9Ih7wwGwi54+NhvX13EEwPZnp0MoRTkvQ=";
    };
  };
  corfu-terminal = {
    pname = "corfu-terminal";
    version = "501548c3d51f926c687e8cd838c5865ec45d03cc";
    src = fetchgit {
      url = "https://codeberg.org/akib/emacs-corfu-terminal";
      rev = "501548c3d51f926c687e8cd838c5865ec45d03cc";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-0qdSsuPE2s+DVu6yuqebMEt/29ypRlvb7TbvWEMHsjU=";
    };
    date = "10 Aug 2023";
  };
  dired-sidebar = {
    pname = "dired-sidebar";
    version = "3bc8927ed4d14a017eefc75d5af65022343e2ac1";
    src = fetchgit {
      url = "https://github.com/jojojames/dired-sidebar";
      rev = "3bc8927ed4d14a017eefc75d5af65022343e2ac1";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-5NZDHDKiiHzBMapEDv4rRGrKXY9VS9js5l28eaouvW0=";
    };
    description = "Sidebar for Emacs leveraging Dired";
    date = "12 Feb 2025";
  };
  gptel = {
    pname = "gptel";
    version = "v0.9.9.3";
    src = fetchFromGitHub {
      owner = "karthink";
      repo = "gptel";
      rev = "v0.9.9.3";
      fetchSubmodules = false;
      sha256 = "sha256-yWPNgifEckRi42xcj7e4iPHgOPIQU8z6dLvGAAKYDSA=";
    };
  };
  helm-sly = {
    pname = "helm-sly";
    version = "3691626c80620e992a338c3222283d9149f1ecb5";
    src = fetchFromGitHub {
      owner = "emacs-helm";
      repo = "helm-sly";
      rev = "3691626c80620e992a338c3222283d9149f1ecb5";
      fetchSubmodules = false;
      sha256 = "sha256-faIdZtWgDN6sz2/srTEZ+2sW/qNz0T2nVkeDHLrnqBs=";
    };
    date = "05 Feb 2021";
  };
  ibuffer-sidebar = {
    pname = "ibuffer-sidebar";
    version = "fb685e1e43db979e25713081d8ae4073453bbd5e";
    src = fetchgit {
      url = "https://github.com/jojojames/ibuffer-sidebar";
      rev = "fb685e1e43db979e25713081d8ae4073453bbd5e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-ww4Npzgz/dnmGCU83WfcEyyTZYKjtoCdJdLv+uw7qBM=";
    };
    description = "A sidebar for IBuffer";
    date = "08 May 2021";
  };
  jinx = {
    pname = "jinx";
    version = "2.5";
    src = fetchFromGitHub {
      owner = "minad";
      repo = "jinx";
      rev = "2.5";
      fetchSubmodules = false;
      sha256 = "sha256-znfsp417JqnBDHmNc0/M16p3znei9tRMoEXtLNN+guI=";
    };
  };
  lsp-mode = {
    pname = "lsp-mode";
    version = "f7fca9db34d202a89da1670a3a0cdf326d42e630";
    src = fetchFromGitHub {
      owner = "emacs-lsp";
      repo = "lsp-mode";
      rev = "f7fca9db34d202a89da1670a3a0cdf326d42e630";
      fetchSubmodules = false;
      sha256 = "sha256-IrAZTpYvV+S24ZAOluF7aBB3ys8bUrsSVdyNtas1nd8=";
    };
    date = "30 Dec 2025";
  };
  maple-iedit = {
    pname = "maple-iedit";
    version = "2d0c819b767a6927bf3a256e9bf8c7dbf8570b9a";
    src = fetchFromGitHub {
      owner = "honmaple";
      repo = "emacs-maple-iedit";
      rev = "2d0c819b767a6927bf3a256e9bf8c7dbf8570b9a";
      fetchSubmodules = false;
      sha256 = "sha256-JEQiAQFEjYxlgyBQVWKiF9PmpaAlMEaqGeHWrrf0n58=";
    };
    description = "Emacs multiple cursors support based on iedit";
    date = "19 Sep 2024";
  };
  nerd-icons-corfu = {
    pname = "nerd-icons-corfu";
    version = "v1.0.0";
    src = fetchFromGitHub {
      owner = "LuigiPiucco";
      repo = "nerd-icons-corfu";
      rev = "v1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-QayHcSE786PcvJ+I6Yx86awqqn7D6rJ7sRBnjnkh1ww=";
    };
  };
  nix-mode = {
    pname = "nix-mode";
    version = "719feb7868fb567ecfe5578f6119892c771ac5e5";
    src = fetchgit {
      url = "https://github.com/NixOS/nix-mode";
      rev = "719feb7868fb567ecfe5578f6119892c771ac5e5";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-W/bxCAn4cvzKGbhILxtTiWJ5Wln2w6g4TPgk+ziCXsQ=";
    };
    date = "21 Apr 2023";
  };
  nix-ts-mode = {
    pname = "nix-ts-mode";
    version = "706bbfaf7f344d7c5914bf7aa5beb2d904eb53b3";
    src = fetchgit {
      url = "https://github.com/nix-community/nix-ts-mode";
      rev = "706bbfaf7f344d7c5914bf7aa5beb2d904eb53b3";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Tc/xDzqh9MNpXcn3LGHPPM28Q5f3qvhudLULbXY4jWE=";
    };
    date = "25 Nov 2025";
  };
  nixpkgs-fmt = {
    pname = "nixpkgs-fmt";
    version = "1f6fb42a5439589c44d99c661cc76958520323cc";
    src = fetchFromGitHub {
      owner = "purcell";
      repo = "emacs-nixpkgs-fmt";
      rev = "1f6fb42a5439589c44d99c661cc76958520323cc";
      fetchSubmodules = false;
      sha256 = "sha256-7Kxho74Av7FBvdlWXTD4dAw7Nl3pdY3j4C8MBq+vnTU=";
    };
    date = "06 Nov 2024";
  };
  orderless = {
    pname = "orderless";
    version = "1.5";
    src = fetchFromGitHub {
      owner = "oantolin";
      repo = "orderless";
      rev = "1.5";
      fetchSubmodules = false;
      sha256 = "sha256-+z17BBbg+7CTdkt1R7381+rVW9iYDQsOzU7vJKqi8zE=";
    };
  };
  org-pretty-table = {
    pname = "org-pretty-table";
    version = "38e4354bbf7a8d08294babd067fac697038119b1";
    src = fetchFromGitHub {
      owner = "Fuco1";
      repo = "org-pretty-table";
      rev = "38e4354bbf7a8d08294babd067fac697038119b1";
      fetchSubmodules = false;
      sha256 = "sha256-IeHnoB9hpISwuzI+pKiU+5wvGvz9YJrG5amGg6DGgq4=";
    };
    date = "19 Mar 2023";
  };
  org-protocol-capture-html = {
    pname = "org-protocol-capture-html";
    version = "a912aaefae8abdada2b2479aec0ad53fcf0b57bf";
    src = fetchFromGitHub {
      owner = "alphapapa";
      repo = "org-protocol-capture-html";
      rev = "a912aaefae8abdada2b2479aec0ad53fcf0b57bf";
      fetchSubmodules = false;
      sha256 = "sha256-X+t8E+0Bxtl6YMU0SwA/vRD8kn1lSXd2PcBhIY001Xo=";
    };
    date = "05 Oct 2022";
  };
  rainbow-delimiters = {
    pname = "rainbow-delimiters";
    version = "f40ece58df8b2f0fb6c8576b527755a552a5e763";
    src = fetchFromGitHub {
      owner = "Fanael";
      repo = "rainbow-delimiters";
      rev = "f40ece58df8b2f0fb6c8576b527755a552a5e763";
      fetchSubmodules = false;
      sha256 = "sha256-JcNRW0+L65NtOBronwyT7SHP9QsNa3ysj4cHv88yfoI=";
    };
    description = "Emacs rainbow delimiters mode";
    date = "30 Aug 2023";
  };
  sly = {
    pname = "sly";
    version = "b01993cf1d6626d541998a77dd802483c9687789";
    src = fetchFromGitHub {
      owner = "joaotavora";
      repo = "sly";
      rev = "b01993cf1d6626d541998a77dd802483c9687789";
      fetchSubmodules = false;
      sha256 = "sha256-uwu5RqKr4/FDGevW5xuMQfkef7tPwz6oKuChx/qcWkI=";
    };
    date = "12 Dec 2025";
  };
  sly-asdf = {
    pname = "sly-asdf";
    version = "6f9d751469bb82530db1673c22e7437ca6c95f45";
    src = fetchFromGitHub {
      owner = "mmgeorge";
      repo = "sly-asdf";
      rev = "6f9d751469bb82530db1673c22e7437ca6c95f45";
      fetchSubmodules = false;
      sha256 = "sha256-IKAqgByGd+HYcuTxscndvbQjP7Jn3jWy0w13ESCwqi8=";
    };
    date = "19 Nov 2022";
  };
  sly-macrostep = {
    pname = "sly-macrostep";
    version = "5113e4e926cd752b1d0bcc1508b3ebad5def5fad";
    src = fetchFromGitHub {
      owner = "joaotavora";
      repo = "sly-macrostep";
      rev = "5113e4e926cd752b1d0bcc1508b3ebad5def5fad";
      fetchSubmodules = false;
      sha256 = "sha256-GVDlwWUIT/IiTPqDQ3w54Tsz6NNl2bMtB3Y4Yh8Srts=";
    };
    date = "11 Dec 2019";
  };
  sly-named-readtables = {
    pname = "sly-named-readtables";
    version = "a5a42674ccffa97ccd5e4e9742beaf3ea719931f";
    src = fetchFromGitHub {
      owner = "joaotavora";
      repo = "sly-named-readtables";
      rev = "a5a42674ccffa97ccd5e4e9742beaf3ea719931f";
      fetchSubmodules = false;
      sha256 = "sha256-mJTXfxrYN3nWxv+/XqwqXiIdZ/YGC30poxn/nEJoWpk=";
    };
    date = "13 Oct 2019";
  };
  sly-overlay = {
    pname = "sly-overlay";
    version = "345b554ad005421b447b2e64c34314ee79f5f8b9";
    src = fetchFromGitHub {
      owner = "fosskers";
      repo = "sly-overlay";
      rev = "345b554ad005421b447b2e64c34314ee79f5f8b9";
      fetchSubmodules = false;
      sha256 = "sha256-WYhwbwmCSfUifF3gCOe22ibo+p6nQGffwkrT9F8D2+8=";
    };
    date = "16 Aug 2025";
  };
  sly-quicklisp = {
    pname = "sly-quicklisp";
    version = "34c73d43dd9066262387c626c17a9b486db07b2d";
    src = fetchFromGitHub {
      owner = "joaotavora";
      repo = "sly-quicklisp";
      rev = "34c73d43dd9066262387c626c17a9b486db07b2d";
      fetchSubmodules = false;
      sha256 = "sha256-YIP4yIuYFPXTs3GODcJuf9txDen0LX/3Ouvf9TTTEo8=";
    };
    date = "06 Dec 2021";
  };
  sly-repl-ansi-color = {
    pname = "sly-repl-ansi-color";
    version = "b9cd52d1cf927bf7e08582d46ab0bcf1d4fb5048";
    src = fetchFromGitHub {
      owner = "PuercoPop";
      repo = "sly-repl-ansi-color";
      rev = "b9cd52d1cf927bf7e08582d46ab0bcf1d4fb5048";
      fetchSubmodules = false;
      sha256 = "sha256-HfQu+2THLKMhEJLcSfDdL6uoDYpuHqC+9B79x5ex7Dk=";
    };
    date = "20 Oct 2017";
  };
  tempel = {
    pname = "tempel";
    version = "1.9";
    src = fetchFromGitHub {
      owner = "minad";
      repo = "tempel";
      rev = "1.9";
      fetchSubmodules = false;
      sha256 = "sha256-xSVfOspxWMIeoCQjN5KEtXQqiIJjK9y9FXR/bR2jjrQ=";
    };
  };
  vertico = {
    pname = "vertico";
    version = "2.6";
    src = fetchFromGitHub {
      owner = "minad";
      repo = "vertico";
      rev = "2.6";
      fetchSubmodules = false;
      sha256 = "sha256-Xhc6Py7Cy6yn/nqsuSe1fb67Ql65Yfzpcq4bme7v85g=";
    };
  };
  zetteldeft = {
    pname = "zetteldeft";
    version = "d2ddb3dc89d79c84fb8abd48478f7afe037d3a09";
    src = fetchFromGitHub {
      owner = "EFLS";
      repo = "zetteldeft";
      rev = "d2ddb3dc89d79c84fb8abd48478f7afe037d3a09";
      fetchSubmodules = false;
      sha256 = "sha256-eUwwZsVTNY1ZjIqk/5grpWjhLhy3aNKBD/2kAkQOSzQ=";
    };
    date = "17 Mar 2023";
  };
}
